...
void Renderer2D::gpuRender(const mesh &newObj)
{
    ...
    float time = SDL_GetTicks() * 0.001f; // time in seconds
    // Rotate the model over time (Y axis and X axis rotations)
    // Camera view matrix (translate the model away from camera)
    glm::mat4 viewMat = glm::translate(glm::mat4(1.0f), glm::vec3(0, 0, -5));
    // Projection matrix (perspective projection with 60° FOV)
    glm::mat4 projMat = glm::perspective(glm::radians(60.0f), float(windowWidth) / float(windowHeight), 0.1f, 100.0f);
    // Combine model, view, projection
    glm::mat4 mvpMat = projMat * viewMat * currentTransform;
    glm::mat4 mvMat = viewMat * currentTransform; // model-view (without projection), for culling
    ...
    {
        // Transform vertices by Model-View and Model-View-Projection matrices
        glm::vec4 v0 = mvMat * tri.p[0];
        glm::vec4 v1 = mvMat * tri.p[1];
        glm::vec4 v2 = mvMat * tri.p[2];
        // // Back-face culling (triangle normal facing camera?)
        // glm::vec3 e1 = glm::vec3(v1 - v0);
        // glm::vec3 e2 = glm::vec3(v2 - v0);
        // glm::vec3 normal = glm::normalize(glm::cross(e1, e2));
        // if (glm::dot(normal, glm::vec3(v0)) <= 0.0f)
        // {
        //     // Triangle facing away, skip it
        //     continue;
        // }
        // // Apply projection
        ...
        // Perspective divide (normalize to NDC)
        p0 /= p0.w;
        p1 /= p1.w;
        p2 /= p2.w;
        ...
        {
            return glm::vec2((v.x * 0.5f + 0.5f) * windowWidth,
    ...
    // 3. Sort triangles by depth (far to near) – optional, keeps overlay drawing order neat
    std::sort(triList.begin(), triList.end(), [](const Tri &a, const Tri &b)
              { return a.depth > b.depth; });

    // 4. Prepare triangles for CUDA rasterization
    std::vector<CudaTri> ct;
    ct.reserve(triList.size());
    for (auto &T : triList)
    {
        ct.push_back({T.p[0].x, T.p[0].y,
                      T.p[1].x, T.p[1].y,
                      T.p[2].x, T.p[2].y,
                      T.z[0], T.z[1], T.z[2]});
    }
    // Render frame on GPU and retrieve color+depth buffers
    renderFrame(ct.data(), static_cast<int>(ct.size()),
                cudaPixelBuffer, cudaDepthBuffer);

    // 5. Overlay wireframe edges if needed (using CPU and depth buffer)
    if (mode == WIREFRAME)
    {
        // Clear color buffer to black (for pure wireframe mode)
        std::fill_n(cudaPixelBuffer, windowWidth * windowHeight, 0xFF000000u);
    }
    if (mode == WIREFRAME || mode == SHADED_WIREFRAME)
    {
        // Draw triangle edges with depth-tested Bresenham lines (green lines)
        ...
        for (auto &T : triList)
        {
            // Convert each vertex position to integer pixel coordinates
    ...
    std::memcpy(screenBuffer.data(), // destination for SDL
                cudaPixelBuffer,     // what we just rendered
                ...
                while (std::getline(chunk_ss, segment, '/'))
                {
                    try
                    {
                        if (!segment.empty())
                        {
                            ...
                            {
                                if (part == 0)
                                    index = static_cast<int>(temp_verts.size()) + index + 1;
                                else if (part == 1)
                                    index = static_cast<int>(temp_texCoords.size()) + index + 1;
                                else if (part == 2)
                                    index = static_cast<int>(temp_normals.size()) + index + 1;
                            }
